...
public:
  ...
  using completion_func_t =
      std::function<void(const Napi::Value &, const Napi::Error &)>;

  // Both functions will be called within the same HandleScope

  // Must be called from Node event loop because it calls napi_create_reference
  // and uv_async_init
  ...
  // All other member functions can be called from any thread, including move
  // constructor and destructor
  ThreadSafeCallback(ThreadSafeCallback &&other);
  ~ThreadSafeCallback();

  // Invoke JS callback from any thread. These functions are additionally
  // thread-safe, i.e. they can be invoked concurrently
  ...
  void operator()(arg_func_t arg_function,
  ...
  template <typename T>
  std::future<T>
  call(std::function<T(const Napi::Value &)> completion_function);
  template <typename T>
  std::future<T>
  call(arg_func_t arg_function,
...
#include "napi-thread-safe-callback-impl.h"