...
namespace Gloo::Internal::MicDetector
{
  ...
  template <typename T>
  void OSX_Subscribe(AudioDeviceID device, AudioObjectPropertySelector prop, T *data)
  {
    AudioObjectPropertyAddress props = {
        prop,
        kAudioObjectPropertyScopeGlobal,
        kAudioObjectPropertyElementMain};
    AudioObjectAddPropertyListener(device, &props, &T::OnListener, static_cast<void *>(data));
  }

  template <typename T>
  void OSX_Unsubscribe(AudioDeviceID device, AudioObjectPropertySelector prop, T *data)
  {
    AudioObjectPropertyAddress props = {
        prop,
        kAudioObjectPropertyScopeGlobal,
        kAudioObjectPropertyElementMain};
    AudioObjectRemovePropertyListener(device, &props, &T::OnListener, static_cast<void *>(data));
  }

  class OSXMicrophoneDevice
  {
  public:
    OSXMicrophoneDevice(AudioObjectID id, DeviceManager *owner) : owner_(owner), inUse_(false), deviceId(id)
    {
      UpdateInUse(false);
    }
    ...
    void stop() {
      ...
      if (prev) {
        OSX_Unsubscribe(deviceId, kAudioDevicePropertyDeviceIsRunningSomewhere, this);
      }
    }
    ...
    AudioObjectID id() const
    {
      return deviceId;
    }

    static OSStatus OnListener(AudioObjectID inObjectID,
  ...
  private:
    void startImpl()
    {
      OSX_Subscribe(kAudioObjectSystemObject, kAudioHardwarePropertyDevices, this);
      {
    ...
    void stopImpl()
    {
      OSX_Unsubscribe(kAudioObjectSystemObject, kAudioHardwarePropertyDevices, this);
      {
        std::unique_lock<std::mutex> lk_(m_);
        for(auto& d : devices) {
          d->stop();
    ...
    void UpdateDeviceList()
    {
      ...
      {
        std::unique_lock<std::mutex> lk_(m_);
        std::remove_if(devices.begin(), devices.end(), [&](const auto &item)
                       {
  ...
  MakeDeviceManager(DeviceManager::Callback cb)
  {
