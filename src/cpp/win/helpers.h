...
#define LOG_ERROR(hr_err, expr_string)                                        \
  ...
  std::cout << "FAILED " << __FILE__ << ":" << __LINE__ << " " << expr_string \
            << " ::" << errMsg##__LINE__ << std::endl; \
...
#define QUERYINTERFACE_HELPER() HRESULT STDMETHODCALLTYPE QueryInterface(REFIID iid, void **object)