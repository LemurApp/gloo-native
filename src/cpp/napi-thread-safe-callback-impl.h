...
public:
  Impl(Napi::Reference<Napi::Value> &&receiver,
       Napi::FunctionReference &&callback)
      : receiver_(std::move(receiver)), callback_(std::move(callback)),
        close_(false) {
    if (receiver_.IsEmpty())
      receiver_ = Napi::Persistent(
          static_cast<Napi::Value>(Napi::Object::New(callback_.Env())));
    uv_async_init(uv_default_loop(), &handle_, &static_async_callback);
    handle_.data = this;
  }

  void unref() { uv_unref(reinterpret_cast<uv_handle_t *>(&handle_)); }
...
inline ThreadSafeCallback::ThreadSafeCallback(const Napi::Value &receiver,
                                              const Napi::Function &callback)
    : impl(nullptr) {
  if (!receiver.IsEmpty() && !(receiver.IsObject() || receiver.IsFunction()))
    throw Napi::Error::New(callback.Env(),
...
inline void ThreadSafeCallback::unref() { impl->unref(); }
...
ThreadSafeCallback::operator()(arg_func_t arg_function) {
  auto promise = std::make_shared<std::promise<void>>();
  operator()(arg_function,
             [promise](const Napi::Value &value, const Napi::Error &error) {
               try {
                 if (error.IsEmpty())
                   promise->set_value();
                 else
...
inline void
ThreadSafeCallback::operator()(completion_func_t completion_function) {
  operator()(nullptr, completion_function);
}

inline void
ThreadSafeCallback::operator()(arg_func_t arg_function,
                               completion_func_t completion_function) {
  if (impl)
...
inline void ThreadSafeCallback::error(const std::string &message,
                                      completion_func_t completion_function) {
  operator()(
      [message](napi_env env, std::vector<napi_value> &args) {
        args.push_back(Napi::Error::New(env, message).Value());
      },
      completion_function);
}
...
ThreadSafeCallback::callStringify(arg_func_t arg_function) {
...
inline std::future<std::string>
ThreadSafeCallback::errorStringify(const std::string &message) {
...
template <typename T>
inline std::future<T> ThreadSafeCallback::call(
    std::function<T(const Napi::Value &)> completion_function) {
...
inline std::future<T> ThreadSafeCallback::call(
  ...
  operator()(arg_function,
             [promise, completion_function](const Napi::Value &value,
...
inline void ThreadSafeCallback::call() { operator()(nullptr, nullptr); }
